    ...
    <el-dialog :title="modifyItem ? '编辑账号' : '创建账号'" :visible.sync="dialogVisible" width="500px" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false">
      <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="选择角色" prop="roleId">
          <role-select v-model="ruleForm.roleId" @handleRowChange="handleRowChange" />
        </el-form-item>
...
<script>
import { getAdminAccountUserList, modifyAdminAccountUser, createAdminAccountUser } from '@/api/systemSetting/projectManager'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getAdminAccountUserList(param).then(res => {
    ...
    async handleEdit(row) {
      const { idCard, userName, roleId, remark, nickName, roleName } = row
      this.ruleForm = {
        phone: userName,
        userName: nickName,
        idCard: idCard,
    ...
    deleteManager(roleInfo, params) {
      return modifyAdminAccountUser(Object.assign({}, roleInfo, { ...params }))
    },
    ...
    handleSubmit: debounce(async function() {
      ...
      if (this.modifyItem) {
        promise = modifyAdminAccountUser(params)
        tips = `编辑`
      } else {
        promise = createAdminAccountUser(params)
        tips = `创建`
      }
    ...
    // 获取ajax的入参数据
    getAjaxParams() {
      const { phone, pwd, userName, idCard, roleId, note, roleName } = this.ruleForm
      const result = {
    ...
    handleRowChange(role) {
      this.ruleForm.roleName = role ? role.label : ''
    }
  }
}
</script>
